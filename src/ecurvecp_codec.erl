...
client_codec(SLTPK, #{public := CSTPK, secret := CSTSK}, SE, CE) ->
  #client_codec{
...
decode_curvecp_packet(<<?HELLO, SE:16/binary, CE:16/binary, CSTPK:32/binary,
  ...
  #hello_packet{server_extension=SE, client_extension=CE,
                client_short_term_public_key=CSTPK,
                nonce=Nonce, box=Box};
decode_curvecp_packet(<<?COOKIE, CE:16/binary, SE:16/binary,
...
decode_curvecp_packet(<<?CLIENT_M, CE:16/binary, SE:16/binary,
...
-spec decode_initiate_box(initiate_packet(), key(), key())
  -> {ok, term()} | {error, failed_verification}.
decode_initiate_box(Initiate, PK, SK) ->
  #initiate_packet{nonce=Nonce, box=Box} = Initiate,
  NonceString = ecurvecp_nonces:nonce_string(initiate, Nonce),
  enacl:box_open(Box, NonceString, PK, SK).

-spec verify_cookie(initiate_packet(), [key()]) -> boolean().
verify_cookie(_, []) ->
  false;
...
-spec verify_initiate_box_contents(binary(), key(), key()) -> boolean().
...
-spec verify_vouch(binary(), key(), key(), key()) -> boolean().
...
-spec verify_domain_name(string()) -> boolean().
verify_domain_name(_DomainName) ->
  true.

-spec encode_client_msg_packet(iodata(), key(), key(), key(),
...
-spec encode_client_msg_packet(iodata(), key(), key(),
                               extension(), extension()) -> binary().
